...
<div class="content-body">
	<?php
	table_open('',true,base_url('settings/import_prsn_dpk/data'),'tbl_prsn_dpk');
		thead();
			tr();
...
modal_open('modal-form');
	modal_body();
		form_open(base_url('settings/import_prsn_dpk/save'),'post','form');
			col_init(3,9);
			input('hidden','id','id');
...
modal_open('modal-import',lang('impor'));
	modal_body();
		form_open(base_url('settings/import_prsn_dpk/import'),'post','form-import');
			col_init(3,9);
...
$('.btn-refresh').on('click',function(){
    ...
    cLoading.open();
	$.ajax({
		url 	: base_url+'settings/import_prsn_dpk/check_cabang',
		type	: 'post',
		data 	: {
