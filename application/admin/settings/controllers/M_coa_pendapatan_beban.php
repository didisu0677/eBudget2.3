...
class M_coa_pendapatan_beban extends BE_Controller {

	var $table = 'tbl_m_coa_pendapatan_beban';
	var $kode_anggaran = '';
	...
	function index() {
		$data['tipe'] 	= [['value' => 1, 'name' => lang('pendapatan')],['value' => 2, 'name' => lang('beban')]];
	...
	function delete() {
		$response = destroy_data('tbl_m_coa_pendapatan_beban','id',post('id'));
		render($response,'json');
	}

	function template() {
		ini_set('memory_limit', '-1');
		$arr = ['kode_anggaran' => 'kode_anggaran','coa' => 'coa','nama' => 'nama','tipe' => 'Tipe','keterangan' => 'keterangan','is_active' => 'is_active'];
		$config[] = [
			'title' => 'template_import_m_coa_pendapatan_beban',
			'header' => $arr,
		];
	...
	function import() {
		...
		$col = ['coa','nama','tipe','keterangan','is_active'];
	...
	function export() {
		ini_set('memory_limit', '-1');
		$arr = ['coa' => 'Coa','nama' => 'Nama','tipe' => 'Tipe','keterangan' => 'Keterangan','is_active' => 'Aktif'];
		$data = get_data($this->table,'kode_anggaran',$this->table)->result_array();
		$config = [
			'title' => 'data_m_coa_pendapatan_beban',
			'data' => $data,
			'header' => $arr,
		];
		$this->load->library('simpleexcel',$config);
