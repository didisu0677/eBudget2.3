...
class Import_jml_rekening extends BE_Controller {
	...
	var $table = 'tbl_import_jumlah_rekening';
	function __construct() {
		parent::__construct();
	}

	function index() {
		$access = get_access('import_jml_rekening');
		$data['access']	= $access;
	...
	function save() {
		...
		$response = save_data('tbl_import_jumlah_rekening',$data,post(':validation'));
		...
		foreach($jml as $i => $k) {
			if($i==0) {
				...
				for($j = 2; $j <= $k; $j++) {
					...
					$ck = get_data('tbl_import_jumlah_rekening',[
						'select' 	=> 'id',
						'where' 	=> [
							'no_coa' 		=> $data['no_coa'],
					...
					if($data['no_coa']):
						$save = save_data('tbl_import_jumlah_rekening',$data,false);
						if(isset($save['id'])) $c++;
					endif;
				}
			}
	...
	function export() {
		...
		$arr['is_active'] = 'Aktif';
		$data = get_data('tbl_import_jumlah_rekening','kode_anggaran',user('kode_anggaran'))->result_array();
		$config = [
	...
	function check_cabang(){
		...
		$fields = get_field('tbl_import_jumlah_rekening','name');
		$arrColumn = [];
		...
		if(count($arrColumn)>0):
			...
			$load 	 = 'reload';
			$this->load->dbforge();
			$this->dbforge->add_column('tbl_import_jumlah_rekening',$arrColumn);
		endif;
		render([
			'status' 	=> 'success',
			'message'	=> $message,
