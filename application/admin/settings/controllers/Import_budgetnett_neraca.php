...
class Import_budgetnett_neraca extends BE_Controller {
    var $controller = 'import_budgetnett_neraca';
    var $detail_tahun;
    var $kode_anggaran;
    ...
    function data() {
        ...
        $config['where']['tbl_history_import_budgetnett.page'] = 'neraca';
	...
	function template() {
		$link =  base_url('download/file/'.encode_string(dir_upload('import_budgetnett').'template_import_budgetnett_neraca.xlsx'));
        redirect($link);
	}
        ...
        if($file){
            ...
            if(count($arr_kode_cabang)>0):
                ...
                foreach ($arr_kode_cabang as $v) {
                    $this->db->query('CALL stored_budget_nett_sum_coa_neraca("'.$kode_anggaran.'","'.$v.'")');
                    
                    // perhitungan aktiva dan pasiva
                    $dt_result = get_data('tbl_budget_nett_neraca',[
                        'where' => [
                            'kode_anggaran' => $kode_anggaran,
                            'kode_cabang'   => $v,
                            'coa'           => ['1000000','1800000','2000000','2800000'],
                        ]
                    ])->result_array();
                    ...
                    $dt_pasiva  = [];
                    $dt_pasiva2 = [];
                    foreach($dt_result as $vv){
                        if($vv['coa'] == '1000000'):
                           $dt_aktiva = $vv; 
                        elseif($vv['coa'] == '1800000'):
                            $dt_aktiva2 = $vv;
                        elseif($vv['coa'] == '2000000'):
                            $dt_pasiva = $vv;
                        elseif($vv['coa'] == '2800000'):
                            $dt_pasiva2 = $vv;
                        endif;
                    }
                    ...
                    for ($i=1; $i <= 12 ; $i++) { 
                        ...
                        $val_aktiva     = 0;
                        $val_aktiva2    = 0;
                        if(isset($dt_aktiva[$field])) $val_aktiva = $dt_aktiva[$field];
                        if(isset($dt_aktiva2[$field])) $val_aktiva2 = $dt_aktiva2[$field];

                        $val_pasiva  = 0;
                        $val_pasiva2 = 0;
                        if(isset($dt_pasiva[$field])) $val_pasiva = $dt_pasiva[$field];
                        if(isset($dt_pasiva2[$field])) $val_pasiva2 = $dt_pasiva2[$field];

                        if($val_pasiva>$val_aktiva):
                            $arrSaved['1000000'][$field] = $val_aktiva  + ($val_pasiva-$val_aktiva);
                            $arrSaved['1800000'][$field] = $val_aktiva2 + ($val_pasiva-$val_aktiva);
                            $arrSaved['1801000'][$field] = ($val_pasiva-$val_aktiva);
                        else:
                            $arrSaved['1801000'][$field] = 0;
                        endif;

                        if($val_pasiva<$val_aktiva):
                            $arrSaved['2000000'][$field] = $val_pasiva + ($val_aktiva-$val_pasiva);
                            $arrSaved['2800000'][$field] = $val_pasiva2 + ($val_aktiva-$val_pasiva);
                            $arrSaved['2801000'][$field] = ($val_aktiva-$val_pasiva);
                        else:
                            $arrSaved['2801000'][$field] = 0;
                        endif;

                    }

                    foreach($arrSaved as $coa => $record){
                        update_data('tbl_budget_nett_neraca',$record,[
                            'kode_anggaran' => $kode_anggaran,
                            'kode_cabang'   => $v,
                            'coa'           => $coa
                        ]);
               ...
               $cab_gab = get_data('tbl_m_cabang a',[
                ...
                foreach ($cab_gab as $v) {
                    $this->db->query("CALL stored_budget_nett_cron_neraca('".$kode_anggaran."', '".$v->kode_cabang."', 1)");
                }
            endif;
