...
class Rate extends BE_Controller {
	...
	function get_data() {
		$data = get_data('tbl_rate','id',post('id'))->row_array();
		render($data,'json');
	}
	...
	function save() {
		...
		$response = save_data('tbl_rate',$data,post(':validation'));
		render($response,'json');
	}

	function delete() {
		$response = destroy_data('tbl_rate','id',post('id'));
		render($response,'json');
	}

	function template() {
		ini_set('memory_limit', '-1');
		$arr = ['jenis_rate' => 'jenis_rate','no_coa' => 'no_coa','nama_coa' => 'nama_coa'];
	...
	function import() {
		...
		$col = ['jenis_rate','no_coa','nama_coa'];
		$column = check_column_table(user('kode_anggaran'),$this->table);
		foreach($column as $v){
			array_push($col,$v);
		}
		...
		foreach($jml as $i => $k) {
			if($i==0) {
				...
				if(isset($ls[1])):
					foreach($ls[1] as $v){
						$v = str_replace(' ','_',$v);
						if(in_array(strtolower($v),['no_coa','nama_coa','is_active','jenis_rate'])):
							$v = strtolower($v);
						endif;
						array_push($col,$v);
					}
				...
				for($j = 2; $j <= $k; $j++) {
					...
					if($data['no_coa']):
						$save = save_data($this->table,$data,false);
						if(isset($save['id'])) $c++;
					endif;
				}
			}
	...
	function export() {
		ini_set('memory_limit', '-1');
		$arr = ['jenis_rate' => 'Jenis Rate','no_coa' => 'No Coa','nama_coa' => 'Nama Coa'];
		$column = check_column_table(user('kode_anggaran'),$this->table);
		foreach($column as $v){
			$arr[$v] = $v;
		}
		$arr['is_active'] = 'Aktif';
		$data = get_data('tbl_rate','kode_anggaran',user('kode_anggaran'))->result_array();
		$config = [
			'title' => 'data_rate',
			'data' => $data,
			'header' => $arr,
