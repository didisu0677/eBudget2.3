...
class M_import_budget_nett_konsolidasi_labarugi extends BE_Controller {
	var $controller 	= 'm_import_budget_nett_konsolidasi_labarugi';
	...
	function data() {
        ...
        $config['where']['tbl_history_import_budget_nett_konsolidasi.page'] = 'labarugi';
        ...
        if($file){
                ...
                foreach($loadexcel->getWorksheetIterator() as $worksheet){
                    ...
                    if(!empty($worksheet->getCellByColumnAndRow(0, 2)->getValue()) && is_numeric($worksheet->getCellByColumnAndRow(1, $row)->getValue())) {
	                    ...
	                    if(empty($cek)){
                            $data_save['create_at'] = date("Y-m-d H:i:s");
                            $data_save['create_by'] = user('username');
	                        $save = insert_data($table,$data_save);
	                        if($save) $d++;
	                    }else {
                            $data_save['update_at'] = date("Y-m-d H:i:s");
                            $data_save['update_by'] = user('username');
    ...
    function template(){
        ...
        $config[] = [
            'title'     => 'template_import_budget_nett_kanpus_syariah_labarugi',
            'header'    => $arr,
            'data'      => $this->arrData
        ];
        // render($config,'json');
