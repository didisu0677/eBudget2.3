...
class M_monitoring_anggaran extends BE_Controller {

	var $table = 'tbl_m_monitoring_anggaran';
	var $kode_anggaran = '';
	...
	function get_data() {
		$data = get_data('tbl_m_monitoring_anggaran','id',post('id'))->row_array();
		render($data,'json');
	}
	...
	function delete() {
		$response = destroy_data('tbl_m_monitoring_anggaran','id',post('id'));
		render($response,'json');
	}
	...
	function template() {
		...
		$config[] = [
			'title' => 'template_import_m_monitoring_anggaran',
			'header' => $arr,
		];
	...
	function export() {
		ini_set('memory_limit', '-1');
		$arr = ['coa' => 'Coa','nama' => 'Nama','keterangan' => 'Keterangan','is_active' => 'Aktif'];
		$data = get_data('tbl_m_monitoring_anggaran','kode_anggaran',$this->kode_anggaran)->result_array();
		$config = [
			'title' => 'data_m_monitoring_anggaran',
			'data' => $data,
			'header' => $arr,
		];
		$this->load->library('simpleexcel',$config);
