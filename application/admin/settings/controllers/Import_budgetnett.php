...
class import_budgetnett extends BE_Controller {
    var $controller = 'import_budgetnett';
	var $path = 'settings/adj_budget_nett/';
    var $detail_tahun;
    var $kode_anggaran;
    ...
    function detail($id){
        ...
        if(isset($data['id'])) {
            render($data,'layout:false view:'.$this->path.'detail');
	...
	function template() {
		$link =  base_url('download/file/'.encode_string(dir_upload('import_budgetnett').'template_import_budgetnett.xlsx'));
        redirect($link);
	}
        ...
        if($file){
            ...
            if(count($arr_kode_cabang)>0):
                ...
                foreach ($arr_kode_cabang as $v) {
                    ...
                    $dt_result = get_data('tbl_budget_nett',[
                        'where' => [
                            'kode_anggaran' => $kode_anggaran,
                            'kode_cabang'   => $v,
                    ...
                    foreach($arrSaved as $coa => $record){
                        update_data('tbl_budget_nett',$record,[
                            'kode_anggaran' => $kode_anggaran,
                            'kode_cabang'   => $v,
                            'coa'           => $coa
                        ]);
               ...
               $cab_gab = get_data('tbl_m_cabang a',[
                ...
                foreach ($cab_gab as $v) {
                    $this->db->query("CALL stored_budget_nett_cron('".$kode_anggaran."', '".$v->kode_cabang."', 1)");
                }
            endif;
