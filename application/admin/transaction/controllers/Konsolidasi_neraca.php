...
class Konsolidasi_neraca extends BE_Controller {
    var $path = 'transaction/budget_nett_konsolidasi/';
    var $controller = 'konsolidasi_neraca';
    ...
    function export(){
        ...
        $config[] = [
            'title' => 'Konsolidasi Neraca',
            'header' => $neraca_header[0],
            'data'  => $data,
        ];
        
        $this->load->library('simpleexcel',$config);
        $filename = 'Budget_Nett_Konsolidasi_neraca_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.'_'.date('YmdHis');
    ...
    function index() {
        ...
        $this->session->set_userdata([
            'coa_nett_neraca' => $this->get_list_coa($coa_neraca),
        ]);
    ...
    function create_kons(){
        $kode_anggaran    = user('kode_anggaran');
        $this->db->query("CALL stored_budget_nett_kantor_pusat_neraca('".$kode_anggaran."')");
