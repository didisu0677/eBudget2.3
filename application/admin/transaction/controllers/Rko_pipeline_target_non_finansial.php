...
class Rko_pipeline_target_non_finansial extends BE_Controller {
    var $controller = 'rko_pipeline_target_non_finansial';
    ...
    var $arr_m_target_non_finansial = [];
    ...
    function index() {
        ...
        $this->m_target_lainnya();
        $data['m_target_lainnya'] = $this->arr_m_target_non_finansial;
    ...
    private function m_target_lainnya(){
    	$this->arr_m_target_non_finansial = get_data('tbl_m_target_non_finansial',[
    		'where' => [
    	...
    	$item = '';
    	foreach($this->arr_m_target_non_finansial as $k => $v){
    		$item .= '<option value="'.$v->id.'">'.$v->kode.' - '.$v->nama.'</option>';
    	}
    	return $item;
    }
    ...
    function get_option(){
        ...
        elseif(post('page') == 'dt_target_lainnya'):
        	$item = $this->m_target_lainnya();
        endif;
    ...
    function save(){
        ...
        $id_target_lainnya    = post('id_target_lainnya');
        $biaya_sampai   = post('biaya_sampai');
        $dt_key         = post('dt_key');
        ...
        if($dt_id):
            foreach ($dt_id as $k => $v) {
                ...
                $dt_target_non_f = get_data('tbl_m_target_non_finansial','id',$id_target_lainnya[$k])->row();
                $kode_target = '';
                if($dt_target_non_f):
                    $kode_target = $dt_target_non_f->kode;
                endif;
                ...
                $c = [
                    ...
                    'nama_biaya'    => filter_money($biaya_sampai[$k]),
                    'biaya_sampai'  => filter_money($biaya_sampai[$k]),
                    'kode_target_non_finansial' => $kode_target,
                ];
    ...
    function data($anggaran="", $cabang="", $tipe = 'table'){
        ...
        $ls_nett = [];
        foreach($this->arr_m_target_non_finansial as $k => $v){
        	$arr['where']['kode_target_non_finansial'] = $v->kode;
        	$list = get_data('tbl_rko_pipeline a',$arr)->result();
	        $data['list']     = $list;

            // mengambil nilai dari target lainnya operasional
            $budget_nett = get_data('tbl_target_non_finansial',[
                'where' => [
                    'kode_anggaran' => $ckode_anggaran,
            ...
            for ($i=1; $i <= 12 ; $i++) { 
                ...
                $ls_nett['.d-month-'.str_replace(' ','',$v->kode).'-'.$bln] = custom_format($val);
            }
            if(post('export') == 'export'):
                $data['ls_nett'] = $ls_nett;
                $data['kode']    = $v->kode;
                $data['nama']    = $v->kode.' - '.$v->nama;
                $data['config'][]= $this->export($data);
            else:
                $view = $this->load->view($this->path.$this->controller.'/table',$data,true);
                $response['data']['d-content-'.$v->id] = $view;
            endif;
        }
        if(post('export') == 'export'):
            ...
            $kode_cabang        = post('kode_cabang');
            $kode_cabang_txt    = post('kode_cabang_txt');
            // render($data['config'],'json');exit();
            $this->load->library('simpleexcel',$data['config']);
            $filename = 'Pipeline_target_non_finansial_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.str_replace(' ', '_', $kode_cabang_txt).'_'.date('YmdHis');
            $this->simpleexcel->filename($filename);
            $this->simpleexcel->export();
        else:
            $response['status'] = true;
            $response['ls_nett'] = $ls_nett;
    ...
    function get_data(){
        ...
        $list = get_data('tbl_rko_pipeline a',[
            'select' => 'a.*,b.id as id_target_lainnya',
            'join'  => [
                'tbl_m_target_non_finansial b on a.kode_target_non_finansial = b.kode and a.kode_anggaran = b.kode_anggaran type left'
            ],
            'where'         => [
                'a.kode_anggaran'   => $d->kode_anggaran,
                'a.kode_cabang'     => $d->kode_cabang,
                'a.tahun'           => $d->tahun,
                'a.tipe'            => $this->tipe,
                'a.id'              => post('id')
            ]
    ...
    private function export($data){
        ...
        $header[] = str_replace('$$nama',$nama,lang('perkiraan_giro'));
        ...
        foreach ($arrWeekOfMonth['month'] as $k => $v) {
            for ($i=0; $i < $v ; $i++) { 
                if($i == 0):
                    $bln    = sprintf("%02d", $k);
                    $classnya = '.d-month-'.str_replace(' ','',$kode).'-'.$bln;
                    $val = 0;
