...
class Monthly_performance_rkf_cabang extends BE_Controller {

	var $controller = 'monthly_performance_rkf_cabang';
	...
	function index() {
		...
		$data['kolom'] 				= get_data('tbl_m_rkf_kolom',['where' => 'is_active = 1','order_by' => 'urutan'])->result();
	...
	function data(){
        ...
        if(post('export')):
        	...
        	$config[] = $this->export($data,' sd bulan '.month_lang(rkf_bulan($anggaran->kode_anggaran)));
        	...
        	$response   = array(
	            ...
	            'table2' => $this->load->view('transaction/'.$this->controller.'/table',$data2,true),
    ...
    private function export($data,$p1=''){
        ...
        foreach($data['kolom'] as $v_kolom){
            if(!in_array($v_kolom->lang,['act_status_program_kerja','act_status_progres'])):
                $header[] = lang($v_kolom->lang);
            endif;
