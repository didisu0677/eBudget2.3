...
class Valas_laba_rugi extends BE_Controller {
    ...
    function data ($kode_anggaran="", $kode_cabang=""){
        ...
        $labarugi       = get_data('tbl_m_coa a',$arr)->result();
        ...
        // data core / history
        $data_core = [];
        if($this->history_status && count($labarugi['arr_coa'])>0): 
            $column = 'VAL_'.$kode_anggaran;
            $data_core = get_data_core($labarugi['arr_coa'],$this->arr_tahun_core,$column);
        endif;
    ...
    function save_perubahan() {
        ...
        foreach($data as $coa => $v){
            ...
            save_data('tbl_valas_labarugi',$dataSave,[],true);
        }
    ...
    function export(){
        ...
        $config[] = [
            'title' => 'valas laba rugi ('.get_view_report().')',
            'header' => $header[1],
            'data'  => $data,
        ];
        ...
        $filename = 'valas_laba_rugi_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.str_replace(' ', '_', $kode_cabang_txt).'_'.date('YmdHis');
