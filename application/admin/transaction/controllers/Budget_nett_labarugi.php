...
class Budget_nett_labarugi extends BE_Controller {
    var $path = 'transaction/budget_nett/';
    var $controller = 'budget_nett_labarugi';
    ...
    function export(){
        ...
        $neraca_header = json_decode(post('labarugi_header'));
        $neraca = json_decode(post('labarugi'));
        ...
        $filename = 'Budget_Nett_labarugi_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.str_replace(' ', '_', $kode_cabang_txt).'_'.date('YmdHis');
    ...
    private function load_more_general($page){
        ...
        if(isset($cab[$count])):
            ...
            $neraca_nett = get_data('tbl_budget_nett_labarugi',[
    ...
    function export_datacore(){
        ...
        $coa_labarugi = get_data('tbl_m_coa',[
            'where' => [
                'tipe'          => 2,
                'is_active'     => 1,
                'kode_anggaran' => user('kode_anggaran')
        ...
        $coa_labarugi = coa_labarugi($coa_labarugi);
        ...
        $data['coa'] = $coa_labarugi;

        $header_status = false;
        foreach($coa_labarugi['coa'] as $coa){
            $dt = [];
            for ($i=1; $i <= 12 ; $i++) {
                $bln        = sprintf("%02d", $i);
            ...
            $ls = get_data('tbl_m_cabang b',[
                ...
                'join' => [
                    "tbl_budget_nett_labarugi a on b.kode_cabang = a.kode_cabang and a.kode_anggaran = '$kode_anggaran' and a.coa = '$coa->glwnco' type left"
                ],
                'where' => [
                    'b.is_active'       => 1,
                    'b.status_group'    => 0,
