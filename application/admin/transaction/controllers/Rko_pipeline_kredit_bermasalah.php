...
class Rko_pipeline_kredit_bermasalah extends BE_Controller {
    var $controller = 'rko_pipeline_kredit_bermasalah';
    ...
    private function option_produk_kredit(){
        $ls = get_data('tbl_produk_kredit a',[
            'select'    => 'a.coa,b.glwdes',
            'join'      => [
                "tbl_m_coa b on a.coa = b.glwnco and a.kode_anggaran = b.kode_anggaran"
            ],
            'where'     => [
                'a.is_active' => 1,
                'a.kode_anggaran' => $this->kode_anggaran,
        ...
        $item = '';
        foreach ($ls as $k => $v) {
            $item .= '<option value="'.$v->coa.'">'.$v->coa.' - '.remove_spaces($v->glwdes).'</option>';
        }
        return $item;
    }
    ...
    function get_option(){
        ...
        if(post('page') == 'dt_tipe_dana'):
            $item = $this->create_option('tbl_m_rko_tipe_dana_krd_bermasalah');
        ...
        elseif(post('page') == 'dt_produk_kredit'):
            $item = $this->option_produk_kredit();
        endif;
        ...
        if($dt_id):
            foreach ($dt_id as $k => $v) {
                ...
                $c = [
                    ...
                    'sisa_outstanding'  => insert_view_report(filter_money($sisa_outstanding[$k])),
                ];
    ...
    function data($anggaran="", $cabang="", $tipe = 'table'){
        ...
        $arr = ['select'    => '
                    ...
                    e.glwdes as produk_kredit_name,
                ',];
        if($anggaran) {
        ...
        $arr['join'][] = 'tbl_m_rko_tipe_dana_krd_bermasalah d on d.id = a.id_rko_tipe_dana';
        $arr['join'][] = "tbl_m_coa e on a.produk_kredit = e.glwnco and a.kode_anggaran = e.kode_anggaran type left";
        $arr['where']['a.tipe'] = $this->tipe;
        $arr['where']['a.produk_kredit !='] = '';
        $arr['sort_by'] = 'a.id';
        $arr['sort'] = 'DESC';
    ...
    function get_data(){
        ...
        foreach ($list as $k => $v) {
            ...
            if(!is_array($pic)) $pic = [];
            $v->pic = $pic;
            $v->sisa_outstanding = view_report($v->sisa_outstanding);
    ...
    private function export($data){
        ...
        $header[] = lang('sisa_outstanding').' ('.get_view_report().')';
        $header[] = lang('kolektabilitas');
        $header[] = lang('produk_kredit');
        $header[] = lang('keterangan');
        $header[] = lang('pic');
        $header[] = lang('pelaksanaan');
        $header[] = str_replace('$$nama','PENYELESAIAN KRD BERMASALAH',lang('perkiraan_giro'));
        ...
        foreach($data['list'] as $k => $v){
            ...
            $h = [
                $n,
                $v->keterangan,
                view_report($v->sisa_outstanding),
                $v->tipe_dana_name,
                $v->produk_kredit.' - '.remove_spaces($v->produk_kredit_name),
                $v->nama_cabang,
                $d_pic,
        ...
        $filename = 'Pipeline_penyelsaian_krd_bermasalah_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.str_replace(' ', '_', $kode_cabang_txt).'_'.date('YmdHis');
