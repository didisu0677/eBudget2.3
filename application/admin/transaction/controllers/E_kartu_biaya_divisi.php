...
class E_kartu_biaya_divisi extends BE_Controller {
	...
	function index() {
        ...
        // $data = cabang_divisi($this->controller);
	...
	function data(){
        ...
        $data['coa'] 			= $coa;
        $data['coa_renc'][$sub_coa] = $this->data_query($anggaran,$cabang->kode_cabang,[$sub_coa]);
        $data['coa_renc'][$coa] 	= $this->data_query($anggaran,$cabang->kode_cabang,$arr_coa);
        ...
        if($status_gab):
        	...
        	$data_gab['data_renc'] = $this->data_query($anggaran,$arr_cab,$arr_coa);
        ...
        // kantor pusat
        $data_kanpus['data_renc'] = get_data($this->table_budget,[
        	'where' => [
        		'coa' => $coa,
        		'kode_anggaran' => $anggaran->kode_anggaran,
        		'kode_cabang' => '001'
        ...
        $arr_kode_sub_div = get_arr_cabang_level($kode_anggaran,'00100',['last_level' => true]);
        $data_kanpus['data_real'] = $this->data_real($kode_anggaran,$arr_kode_sub_div,$arr_coa);
        $data_kanpus['data_core'] = get_data_core([$coa],[$tahun],'TOT_001');

        $data['kanpus'] = $data_kanpus;
        $view4 	= $this->load->view('transaction/'.$this->controller.'/kanpus',$data,true);

        $class['#result1 tbody'] 	= $view;
        $class['.d-detail'] 		= $view2;
        $class['#result2 tbody'] 	= $view3;
        $class['#result3 tbody'] 	= $view4;
	...
	private function data_query($anggaran,$kode_cabang,$arr_coa){
    	$coa = " a.coa != '0'";
    	if(count($arr_coa)>0):
    		$coa = " a.coa in (".implode(",", $arr_coa).")";
    	endif;
    	if(is_array($kode_cabang)):
    		$kode_cabang_txt = "a.kode_cabang = 'xxx'";
    		if(count($kode_cabang)>0):
	    		$kode_cabang_txt = " a.kode_cabang in (".implode(",", $kode_cabang).")";
	    	endif;
	    else:
	    	$kode_cabang_txt = "a.kode_cabang = '".$kode_cabang."'";
    	endif;
    	$query = "
    		select 
    			SUM(Z.B_01) AS B_01,
    		...
    		from (
				...
				WHERE ".$kode_cabang_txt." AND a.kode_anggaran = '".$anggaran->kode_anggaran."' AND a.anggaran = 1 AND ".$coa."
				GROUP BY a.coa
				...
				JOIN tbl_m_cabang b ON a.kode_cabang = b.kode_cabang and b.kode_anggaran = '".$anggaran->kode_anggaran."'
				WHERE ".$kode_cabang_txt." AND a.kode_anggaran = '".$anggaran->kode_anggaran."' AND ".$coa."
				GROUP BY a.coa
	...
	function save(){
        ...
        check_save_divisi($this->controller,$kode_anggaran,$kode_cabang);
	...
	function get_data(){
        ...
        // pengecekan save untuk cabang
        check_save_divisi($this->controller,$dt['kode_anggaran'],$dt['kode_cabang'],$this->table,'access_edit');
	...
	function delete(){
		...
		// pengecekan save untuk cabang
        check_save_divisi($this->controller,$dt['kode_anggaran'],$dt['kode_cabang'],$this->table,'access_delete');
	...
	function save_status(){
        ...
        check_save_divisi($this->controller,$kode_anggaran,$kode_cabang,$this->table,'access_edit');
    ...
    function export(){
        ...
        $filename = 'E-KARTU_Biaya_Divisi_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.str_replace(' ', '_', $kode_cabang_txt).'_'.date('YmdHis');
