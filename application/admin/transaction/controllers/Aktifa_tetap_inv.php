...
class Aktifa_tetap_inv extends BE_Controller {
    ...
    var $controller = 'aktifa_tetap_inv';
    var $arrID = [];
    ...
    function index($p1="") { 
        ...
        $data   = data_cabang('Aktifa_tetap_inv');
    ...
    function data($anggaran="", $cabang="", $tipe = 'table') {
        ...
        $a = get_access('aktifa_tetap_inv',$data_finish);
        $access_edit = false;
        ...
        foreach($data['grup'][0] as $m0) {         
            ...
            $where['where']['a.id_kebijakan_asumsi'] = 0;
            ...
            $arr['where']['a.id_kebijakan_asumsi'] = 0;
        ...
        $response   = array(
            ...
            'table_sewa' => $this->load->view($this->path.'akt_inv/table_sewa',$data2,true),
            'edit'       => $access_edit,
    ...
    function get_data() {
        $dt = get_data('tbl_rencana_aset',[
            'where' => [
                'id' => post('id'),
                'id_kebijakan_asumsi' => 0
            ]
        ])->row();
        if(!$dt):
    ...
    function delete(){
        ...
        if($ck):
            // pengecekan save untuk cabang
            check_save_cabang($this->controller,$ck->kode_anggaran,$ck->kode_cabang,'access_edit');
        endif;
        ...
        $response = destroy_data('tbl_rencana_aset',[
            'id' => post('id'),
            'id_kebijakan_asumsi' => 0
        ]);
        create_autorun($ck->kode_anggaran,$ck->kode_cabang,'inv');
        $response['message'] = lang('data_berhasil_dihapus');
        render($response,'json');
    }
    ...
    function data2($kode_anggaran,$kode_cabang){
        ...
        $list = get_data('tbl_rencana_aset a',[
            'select' => '
                a.*,
                sum(ifnull(a.harga,0)) as harga,
                c.kode_cabang as kode_div,
                c.nama_cabang as nama_div
            ',
            'join' => [
                'tbl_kebijakan_asumsi b on b.id = a.id_kebijakan_asumsi',
                'tbl_m_cabang c on c.kode_cabang = b.kode_cabang and c.kode_anggaran = b.kode_anggaran'
            ],
            'where' => [
            ...
            'order_by' => 'a.grup,a.kode_inventaris,b.kode_cabang',
            'group_by' => 'a.grup,a.kode_inventaris'
        ])->result();
        ...
        $data['list'] = $list;
        $data['status_inventaris'] = $status_inventaris;
        $view  = $this->load->view($this->path.'akt_inv/kebijakan_asumsi',$data,true);
    ...
    function create_kebijakan_asumsi($anggaran,$cabang){
        ...
        $list = get_data('tbl_kebijakan_asumsi a',[
            'select' => 'a.*,b.keterangan as nama_grup,b.prefiks',
            'join' => [
                'tbl_grup_asetinventaris b on b.kode = a.grup',
            ],
            'where' => "a.kode_anggaran = '".$anggaran->kode_anggaran."' and a.type = 2 and a.grup != '' and ".$arr_type,
            'sort'  => 'ASC',
            'sort_by' => 'a.id',
        ])->result();
        ...
        foreach ($list as $k => $v) {
            ...
            $ck = get_data('tbl_rencana_aset',[
                'select' => 'id,kode_inventaris',
                'where' => $where,
            ])->row();
            $data_save = $where;
            $data_save['keterangan_anggaran'] = $anggaran->keterangan;
            $data_save['tahun'] = $anggaran->tahun_anggaran;
            $data_save['cabang'] = $cabang->nama_cabang;
            $data_save['grup'] = $v->grup;
            $data_save['nama_grup'] = $v->nama_grup;
            $data_save['nama_inventaris'] = $v->uraian;
            $data_save['is_active'] = 1;
            $data_save['id_kebijakan_asumsi'] = $v->id;
            $val = 0;
            $bln = 1;
            for ($i=1; $i <= 12 ; $i++) { 
            ...
            $data_save['bulan'] = $bln;
            $data_save['jumlah'] = 1;
            $data_save['total'] = checkNumber($val) * 1;
            $data_save['id'] = '';
            if(in_array($v->grup,['E.4','E.5','E.7'])):
                $data_save['kode_inventaris'] = $v->kode_inventaris;
            else:
                $prefiks = $v->prefiks;
                $old_kode_inventaris = '';
                if($ck):
                    $old_kode_inventaris = explode(" ", $ck->kode_inventaris)[0];
                endif;
                ...
                $where_last['kode_inventaris like '] = $prefiks.' %';
                $ck_prefiks = get_data('tbl_rencana_aset',
                    ['select' => 'kode_inventaris','where' => $where_last,'sort_by' => 'kode_inventaris', 'sort' => 'DESC']
                )->row();
                if($prefiks != $old_kode_inventaris && $ck_prefiks):
                    $count   = explode(" ", $ck_prefiks->kode_inventaris);
                    $data_save['kode_inventaris'] = $prefiks.' '.(checkNumber($count[1])+1);
                elseif($prefiks != $old_kode_inventaris):
                    $data_save['kode_inventaris'] = $prefiks.' 1';
                endif;
            endif;
            if($ck):
                $data_save['id'] = $ck->id;
            ...
            $res = save_data('tbl_rencana_aset',$data_save,[],true);
