...
class Rko_aset_dan_inventaris extends BE_Controller {

	var $controller = 'rko_aset_dan_inventaris';
    var $path       = 'transaction/';
    var $sub_menu   = 'transaction/rko_aset_dan_inventaris/sub_menu';
    var $detail_tahun;
    var $kode_anggaran;
    var $tahun_anggaran;
    ...
    function get_sub_menu(){
    	$menu = get_data('tbl_grup_asetinventaris',[
    ...
    function data($anggaran="", $cabang="", $tipe = 'table'){
        $this->checkData($anggaran,$cabang);
        $menu = menu();
        ...
        $arr['where']['id_group_inventaris'] = $last_id;
        $list = get_data('tbl_rko_inventaris a',$arr)->result();
        $data['list']     = $list;
    ...
    function save(){
        ...
        $pic            = post('pic');
        $id_group_inventaris = post('id_group_inventaris');
        $arrID = array();
        if($dt_id):
            foreach ($dt_id as $k => $v) {
                $c = [
                	'id_group_inventaris' => $id_group_inventaris,
                    ...
                    'nama_aset'     => $nama_aset[$k],
                    'harga'         => insert_view_report(filter_money($harga[$k])),
                    'nama_cabang'   => $nama_cabang[$k],
                    'pic'           => $pic[$k],
                ];
                $cek = get_data('tbl_rko_inventaris',[
                    'where'         => [
                        'kode_anggaran'   => $ckode_anggaran,
                        'kode_cabang'     => $kode_cabang,
                        'tahun'           => $tahun,
               ...
               if(!isset($cek->id)) {
                    $c['checkbox'] = '[]';
                    $dt_insert = insert_data('tbl_rko_inventaris',$c);
                    array_push($arrID, $dt_insert);
                }else{
                    update_data('tbl_rko_inventaris',$c,['kode_anggaran'   => $ckode_anggaran,
                        'kode_cabang'     => $kode_cabang,
                        'tahun'           => $tahun,
                        'id_group_inventaris' => $id_group_inventaris,
                        'id' => $dt_id[$k]]);
        ...
        if(count($arrID)>0 && post('id')):
            delete_data('tbl_rko_inventaris',['kode_anggaran'=>$ckode_anggaran,'id not'=>$arrID,'kode_cabang'=>$kode_cabang,'tahun'=>$tahun,'id_group_inventaris' => $id_group_inventaris]);
        elseif(post('id')):
            delete_data('tbl_rko_inventaris',['kode_anggaran'=>$ckode_anggaran,'kode_cabang'=>$kode_cabang,'tahun'=>$tahun,'id_group_inventaris' => $id_group_inventaris]);
        endif;
    ...
    function save_perubahan() {       
        ...
        foreach($data as $id => $record) {          
            update_data('tbl_rko_inventaris',$record,'id',$id); }
    }
    function save_checkbox(){
        ...
        try {
            $id     = $d[1];
            $key    = $d[2];
            $row = get_data('tbl_rko_inventaris',[
                'select'    => 'kode_cabang,checkbox',
                'where'     => "id = '".$d[1]."' and id_group_inventaris = '".$id_group_inventaris."'"
            ])->row();
            ...
            $x[$key] = $val;
            update_data('tbl_rko_inventaris',['checkbox' => json_encode($x)],'id',$id);
    ...
    function delete() {
        $response = destroy_data('tbl_rko_inventaris',['id' => post('id')]);
        render($response,'json');
    }

    function get_data(){
        $d = get_data('tbl_rko_inventaris',[
            'where'         => [
                'id'    => post('id'),
            ],
        ])->row();

        $list = get_data('tbl_rko_inventaris',[
            'where'         => [
