...
class Rekap_target_non_finansial extends BE_Controller {
	...
	function index() {
		...
		$data['coa'] 	= get_data('tbl_m_target_non_finansial',[
			'select' => 'id,kode,nama',
			'where'  => [
	...
	function data($kode_anggaran,$target){
		...
		$target   = get_data('tbl_m_target_non_finansial','id',$target)->row();
		if(!$anggaran):
			render(['status' => false,'message' => 'anggaran not found'],'json');exit();
		elseif(!$anggaran):
			render(['status' => false,'message' => 'target not found'],'json');exit();
		endif;
		$list = get_data($this->table,[
			'where' => [
				'kode_target' => $target->kode,
			]
		])->result_array();
	...
	function export(){
        ...
        $config[] = [
            'title' => 'Rekap Target Non Finansial',
            'header' => $header[0],
            'data'  => $data,
        ];
        ...
        $filename = 'Rekap_target_non_finansial_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.str_replace(' ', '_', $coa_txt).'_'.date('YmdHis');
