...
class Plan_input_akt extends BE_Controller {
    ...
    function index($p1="") { 
        ...
        $data = $data = cabang_divisi('plan_input_akt');
    ...
    function data($anggaran="", $cabang="", $tipe = 'table') {
        ...
        $access = get_access('plan_input_akt',$data_finish);
        $access_edit = false;
        ...
        check_access_divisi($this->controller,$anggaran->kode_anggaran,$ckode_cabang,$access);
        ...
        foreach($data['grup'][0] as $m0) {         
            ...
            foreach ($produk as $m1) {
                ...
                $cabang = get_data('tbl_m_cabang',[
                    'where' => "kode_cabang = '$ckode_cabang' and kode_anggaran = '".$anggaran->kode_anggaran."'"
                ])->row();
    ...
    function save() {
        ...
        $anggaran       = get_data('tbl_tahun_anggaran','kode_anggaran',$ckode_anggaran)->row();
        $cabang         = get_data('tbl_m_cabang',[
            'where' => "kode_cabang = '$kode_cabang' and kode_anggaran = '$ckode_anggaran'"
        ])->row();
        $tahun          = $anggaran->tahun_anggaran;
        ...
        // pengecekan save divisi
        check_save_divisi($this->controller,$ckode_anggaran,$kode_cabang,'tbl_rencana_aset');
    ...
    function delete(){
        ...
        $access = get_access('plan_input_akt');
        $edit   = false;
        if($ck && $access['access_edit']):
            $edit = true;
        ...
        // pengecekan save divisi
        check_save_divisi($this->controller,$ck->kode_anggaran,$ck->kode_cabang,'tbl_rencana_aset','access_delete');

        $response = destroy_data('tbl_rencana_aset','id',post('id'),'','',$this->controller);
