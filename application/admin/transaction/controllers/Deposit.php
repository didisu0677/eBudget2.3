...
class Deposit extends BE_Controller {
    ...
    function data3($kode_anggaran="", $kode_cabang=""){
        ...
        foreach ($data['detail_tahun'] as $k => $v) {
            $t[$v['tahun']][] = $v['bulan'];
            if(!in_array($v['tahun'],$arrTahun)):
                array_push($arrTahun,$v['tahun']);
            endif;
        }
    ...
    //    die;
        ...
        $jml_plandep    = [];
        $arr_non5       = [];
        foreach($arrTahun as $v){
            $jml_plandep[$v] = get_data('tbl_budget_plan_deposito a',[
                'select'    => 'a.*',
                'where'     => [
                    'a.kode_anggaran' => $kode_anggaran,
                    'a.kode_cabang' => $kode_cabang,
            ...
            $arr_non5[$v] = get_data('tbl_budget_plan_deposito a',[
        ...
        $data['jml_plandep']    = $jml_plandep;
        $data['jml_non5']       = $arr_non5;
        ...
        if($check_first_data>0):
            $view_segment = $this->rinc_segment($anggaran,$cab,$access_edit);
    ...
    private function rinc_segment($anggaran,$cabang,$access_edit){
    ...
    function data4($kode_anggaran="", $kode_cabang=""){
        ...
        $view_segment = $this->jum_segment($anggaran,$cab,$access_edit);
    ...
    private function jum_segment($anggaran,$cabang,$access_edit){
        $detail_tahun   = get_data('tbl_detail_tahun_anggaran a',[
            'select'    => 'a.bulan,a.tahun,a.sumber_data,b.singkatan',
            'join'      => 'tbl_m_data_budget b on b.id = a.sumber_data',
            'where'     => [
    ...
    function export(){
        ...
        $config[] = [
            'title' => 'Deposito'.' ('.get_view_report().')',
            'header' => $header,
            'data'  => $data,
        ];
        ...
        $config[] = [
            'title' => lang('rin_deposito'),
            'header' => $header,
            'data'  => $data,
        ];

        $title = lang('rincian_segmen_deposito');
        $data = [];
        $header = $dt['#tbl_segment']['header'][0];
        $detail = ['',''];
        foreach($dt['#tbl_segment']['header'][1] as $v){
            $detail[] = $v;
        }
        $detail[] = '';
        $detail[] = '';
        $data[] = $detail;
        foreach(['#tbl_segment'] as $name){
            if(isset($dt[$name])):
                $count2 = 0;
                foreach($dt[$name]['data'] as $k => $v){
                    $count2 = count($v);
        ...
        $title = lang('jum_rek_noa_deposito');
        $data = [];
        ...
        $filename = 'deposito_'.str_replace(' ', '_', $kode_anggaran_txt).'_'.str_replace(' ', '_', $kode_cabang_txt).'_'.date('YmdHis');
