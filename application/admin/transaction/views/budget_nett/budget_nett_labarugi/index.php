		...
		<div class="float-right">
    		...
    		<?php
				...
				$arr = [
                    ...
                    ['btn-export-datacore','Export to Data Core','fa-upload'],
			...
			<div class="col-sm-12 col-12">
				<div class="card">
					...
					<div class="card-body div-body">
						<p class="d-note"><b><?= lang('silahkan_memilih_cabang_lalu_pilih') ?></b></p>
					</div>	
				</div>
			</div>
		</div>	
...
<?php 
    modal_open('modal-export','Export to Data Core','','modal-info');
        modal_body();
            form_open();
                col_init(3,9);
                select('Bulan','bln','required',$arr_bulan,'value','name',1);
                echo '<div class="form-group row">
                <div class="col-sm-9 offset-sm-3">
                <button type="button" class="btn btn-save-export btn-info">Simpan</button>
...
$(document).on('click','.btn-export',function(){
    ...
    var cabang = $('#filter_cabang').val();
    var dt_laba_rugi = get_data_table('#div-'+cabang);
...
$(document).on('click','.btn-export-datacore',function(){
    $('#modal-export').modal('show');
})

$(document).on('click','.btn-save-export',function(){
    var hashids = new Hashids(encode_key);
    var x = hashids.decode($('meta[name="csrf-token"]').attr('content'));
    ...
    var post_data = {
        "kode_anggaran" : $('#filter_anggaran option:selected').val(),
        "kode_anggaran_txt" : $('#filter_anggaran option:selected').text(),
        "bulan"         : $('#bln option:selected').val(),
        "csrf_token"    : x[0],
    }
    var url = base_url + 'transaction/'+controller+'/export_datacore';
    $.redirect(url,post_data,"","_blank");
})
</script>