<?php
	...
	$this->arr_coa_renc[$sub_coa] = [];
	if(count($ls_detail)>0):
		$val_before = 0;
		for ($i=1; $i <=12 ; $i++) { 
			$field 	= 'B_' . sprintf("%02d", $i);
			$val = 0;
			if(isset($coa_renc[$sub_coa][$field])):
				$val = $coa_renc[$sub_coa][$field];
			endif;
			$val_before += checkNumber($val);
			$this->arr_coa_renc[$sub_coa][$field] = $val_before;
		}	
	endif;
	foreach ($ls_detail as $k => $v) {
		...
		$renc  = checkNumber($this->arr_coa_renc[$sub_coa][$month]);
