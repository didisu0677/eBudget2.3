				...
				foreach ($detail_tahun as $k2 => $v2) {
					...
					for ($i=1; $i <=5 ; $i++) { 
						...
						elseif($i == 4):
							...
							$item_4 .= '<td style="background:'.$bgedit.'"><div style="background:'.$bgedit.'" style="min-height: 10px; width: 50px; overflow: hidden;"  contenteditable="'.$contentedit.'" class="edit-value text-right" data-name="'.$v_field.'_'.$i.'" data-id="'.$id.'" data-value="'.custom_format($col_4).'">'.custom_format($col_4).'</div></td>';
						elseif($i == 5):
							$col_5 = ($npl*0.2);
							if(in_array($v_field.'_'.$i, $changed)):
								$col_5 = $d[$v_field.'_'.$i];
							endif;
							$item_5 .= '<td style="background:'.$bgedit.'"><div style="background:'.$bgedit.'" style="min-height: 10px; width: 50px; overflow: hidden;"  contenteditable="'.$contentedit.'" class="edit-value text-right" data-name="'.$v_field.'_'.$i.'" data-id="'.$id.'" data-value="'.custom_format($col_5).'">'.custom_format($col_5).'</div></td>';
						endif;
					}
