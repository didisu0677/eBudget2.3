		...
		<div class="table-responsive tab-pane fade active show" id="result2">
			<table class="table table-striped table-bordered table-app table-hover">
				...
				foreach ($detail_tahun as $k2 => $v2) {
					...
					$arrTotalNpl[$v2->sumber_data.'-'.$v2->tahun][$v_field] = $total_npl;
				...
				foreach ($arrTotalNpl as $k => $data_npl) {
					...
					$ck_data = get_data($table_npl,[
						'select' => 'id',
						'where'	 => "kode_cabang = '$current_cabang' and sumber_data = '$sumber_data' and tahun_core = '$tahun_core' and kode_anggaran = '$anggaran->kode_anggaran' and tahun = '$anggaran->tahun_anggaran' and tipe = '3'"
					])->row();
					if($ck_data):
