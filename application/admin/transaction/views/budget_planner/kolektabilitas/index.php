		...
		<div class="row">
			<div class="col-sm-6">
				<div class="card">
		    		...
		    		<div class="card-body">
		    			<div class="table-responsive tab-pane fade active show tbl-produktif">
						<?php
		...
		<div class="row">
			<div class="col-sm-12 d-input-npl">
				<div class="card mt-3">
					...
					<div class="card-body">
						<div class="table-responsive tab-pane fade active show tbl-input-npl">
							<?php
							table_open('table table-striped table-bordered table-app table-hover');
								thead('sticky-top');
									tr();
									...
									foreach ($detail_tahun as $k => $v) {
										...
										th($column,'','class="text-center align-middle w-bulan"');
									}
								tbody();
								tfoot();
							table_close();
	...
	xhr_ajax = $.ajax({
		...
		success	: function(response) {
			...
			if(!response.status){
				...
				$('.tbl-produktif tbody').html('');
				$('.tbl-konsumtif tbody').html('');
				$('.d-detail').html('');
				$('.d-total-kredit').html('');
				$('.d-produktif-sum').html('');
				return false;
			}
...
$(document).on('click','.btn-export',function(){
    ...
    var classnya = ['.tbl-produktif','.tbl-konsumtif','.tbl-input-npl','.d-total-kredit','.d-produktif-sum'];
    var dt = {};
    ...
    var tot_prod = 0;
    var tot_kons = 0;
    $.each($('.d-detail').find('.card'),function(k,v){
    	var tipe = $(v).attr('data-tipe');
    	if(tipe == 1){
    		tot_prod += 1;
    		var key = 'detail_1_'+tot_prod;
    	}else{
    		tot_kons += 1;
    		var key = 'detail_2_'+tot_kons;
    	}
    ...
    var url = base_url + 'transaction/kolektibilitas/export';
