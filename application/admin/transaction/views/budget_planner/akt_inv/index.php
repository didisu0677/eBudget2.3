...
<div class="content-header page-data" data-additional="<?= $access_additional ?>">
	<div class="main-container position-relative">
		...
		<div class="float-right">
			...
			if (in_array(user('id_group'), id_group_access('aktifa_tetap_inv'), TRUE)){
...
<?php $this->load->view($path.'sub_menu'); ?>
	<div class="main-container mt-3">
							...
							table_open('table table-striped table-bordered table-app table-hover tbl-inv',false,'','','data-table="tbl_m_produk"');
								thead('sticky-top');
									...
									tr();
										...
										th(lang('keterangan'),'','rowspan="2" class="wd-keterangan text-center align-middle"');
										th(lang('catatan'),'','rowspan="2" class="wd-catatan text-center align-middle"');
										th('Harga','','class="text-center wd-harga"');
										th('Jumlah','','class="text-center wd-harga"');
										th('Bulan','','rowspan="2" class="text-center align-middle wd-bln"');
										th('Total','','rowspan="2" class="text-center align-middle wd-harga"');
										th('&nbsp;','','rowspan="2" class="text-center align-middle wd-bln"');
										th('Status','','rowspan="2" class="text-center align-middle wd-catatan"');
									tr();
	    ...
	    <div class="row mt-3">
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header text-center">Aset Sewa (PSAK 73)</div>
	    			<div class="card-body">
	    				<div class="table-responsive tab-pane fade active show height-window">
	    					<?php
							$thn_sebelumnya = user('tahun_anggaran') -1;
							table_open('table table-striped table-bordered table-app table-hover tbl-sewa',false);
								thead('sticky-top');
									...
									tr();
										...
										th(lang('catatan'),'','rowspan="2" class="wd-catatan text-center align-middle"');
										th('Harga','','class="text-center wd-harga"');
										th('Jangka Waktu (Bulan)','','class="text-center wd-harga"');
										th('Bulan','','rowspan="2" class="text-center align-middle wd-bln"');
										th('Total','','rowspan="2" class="text-center align-middle wd-harga"');
										th('&nbsp;','','rowspan="2" class="text-center align-middle wd-bln"');
										th('Status','','rowspan="2" class="text-center align-middle wd-catatan"');
...
modal_open('modal-form','','modal-lg w-90-per',' data-openCallback="formOpen"');
	modal_body();
		form_open(base_url('transaction/aktifa_tetap_inv/save'),'post','form'); 
				col_init(2,4);
				input('hidden','id','id');
...
function getData() {
	...
	var page = base_url + 'transaction/aktifa_tetap_inv/data';
	...
	$.ajax({
		...
		success	: function(response) {
			...
			if(response.autorun>0){
				var url = base_url+"transaction/formula_aktiva_inv/";
...
function loadData2(){
	...
	page = base_url+'transaction/aktifa_tetap_inv/data2';
	page += '/'+kode_anggaran+'/'+kode_cabang;
	cLoader.open(lang.memuat_data + '...');
	if( xhr_ajax2 != null ) {
        xhr_ajax2.abort();
        xhr_ajax2 = null;
...
function save_perubahan() {
	...
	$.ajax({
		url : base_url + 'transaction/aktifa_tetap_inv/save_perubahan',
		data 	: {
			'json' : jsonString,
			kode_cabang : cabang,
