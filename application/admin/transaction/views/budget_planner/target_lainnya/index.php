<div class="content-header page-data" data-additional="<?= $access_additional ?>">
	<div class="main-container position-relative">
		...
		<div class="float-right">
			...
			if (in_array(user('id_group'), id_group_access($this->controller), TRUE)){
				...
				if($access_additional):
					$arr[] = ['btn-import','Import Data','fa-upload'];
				    $arr[] = ['btn-act-template','Template Data','fa-file-alt'];
				endif;
...
modal_open('modal-import',lang('impor'));
	modal_body('mul');
		form_open(base_url('transaction/'.$controller.'/import'),'post','form-import','class="style-select2"');
			col_init(3,9);
...
$('.btn-act-template').on('click',function(){
	window.open(base_url+'transaction/'+controller+'/template', '_blank');
})
$(document).on('click','.btn-export',function(){
	var hashids = new Hashids(encode_key);
    var x = hashids.decode($('meta[name="csrf-token"]').attr('content'));
    var cabang = $('#filter_cabang').val();
    var dt_table = get_data_table('.d-content');
