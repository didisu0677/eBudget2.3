<?php
	...
	foreach ($detail_tahun as $k => $v) {
		...
		$val 	= $jum_trakhir;
		if($index_kali):
			$val = checkNumber($bln_before )+ checkNumber($index_kali);
		endif;
		$key 	= multidimensional_search($giro, array(
			'tahun_core' => $v->tahun,
			'coa' => '2101012',
	...
	foreach (['212' => 'Retail','211' => 'Korporasi'] as $coa => $name) {
		...
		foreach ($detail_tahun as $k => $v) {
			...
			$field  = 'P_' . sprintf("%02d", $v->bulan);
			$bulan 	= sprintf("%02d", $v->bulan);
			$val 	= (float) $jum_trakhir;
			if($index_kali):
				$val = (float) $bln_before + (float) $index_kali;
			endif;
			$key 	= multidimensional_search($giro, array(
				'tahun_core' => $v->tahun,
				'coa' => $coa,
			...
			if(strlen($key)>0):
				$is_edit = json_decode($giro[$key]['is_edit'],true);
				if(isset($is_edit[$v->tahun.$bulan])):
					$val = (float) $is_edit[$v->tahun.$bulan];
					$jum_trakhir = $val;
				endif;
			endif;
			$bln_before = $val;
	...
	foreach ($detail_tahun as $k => $v) {
		$val = (float) $arr_non_kasda[$v->tahun.$v->bulan];
			
		if(!isset($arr_total[$v->tahun.$v->bulan])):
			$arr_total[$v->tahun.$v->bulan] = 0;
		endif;	

		$val2 = $arr_total[$v->tahun.$v->bulan];
		if (is_numeric($val2) && is_numeric($val)) {
		  	$arr_total[$v->tahun.$v->bulan] += $val;
		}else{
			$arr_total[$v->tahun.$v->bulan] = $val;
		}
