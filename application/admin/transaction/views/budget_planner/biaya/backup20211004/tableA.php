...
foreach ($coa as $k => $v) {
	...
	for ($i=1; $i <= 12 ; $i++) { 
		...
		$dataSaved['bulan_b'.$i] = $val;
		$item .= '<td class="'.ck_edited($v->cabang_edit,'bulan_b'.$i).'" '.$tr.'><div style="min-height: 10px; width: 100%; overflow: hidden;" s contenteditable="false" class="edit-value text-right" data-name="bulan_b'.$i.'" data-id="'.$v->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus).'</div></td>';
		$biaya_tahun1 += $val;
	}
	checkForUpdate($dataSaved);
