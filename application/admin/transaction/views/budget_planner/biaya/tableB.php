...
$table ='<tr><td class="border-none bg-white white">.</td></tr><tr><td class="border-none bg-white white">.</td></tr>';
$table2 = '<tr style="margin-top:30px" class="tr-gray">
            ...
            for ($i = 1; $i <= 12; $i++) {
				...
				$table2 .= '<th class="text-center" style="min-width:80px;">'.$column.'</th>';
			}
 	$table2 .='<th class="border-none bg-white"></th>
            <th class="text-center" style="min-width:80px;">BIAYA PERBULAN</th>
            <th class="text-center" style="min-width:80px;">BIAYA PERTAHUN</th>
            <th class="border-none bg-white"></th>
...
if(isset($coa_ket)):
	$item .= '<tr>';
	$item .= '<td><b>'.$coa_ket->glwnob.'</b></td>';
	$item .= '<td><b>'.$coa_ket->glwnco.'</b></td>';
	$item .= '<td><b>'.remove_spaces($coa_ket->glwdes).'</b></td>';
	for ($i=1; $i <=12 ; $i++) {
		$item .= '<td></td>';
	}
	$item .= '</tr>';
...
foreach ($coa as $k => $v) {
	...
	if(isset($detail['1'][$v->glwnco])){
		foreach ($detail['1'][$v->glwnco] as $k2 => $v2) {
				...
				foreach ($detail['2'][$v2->glwnco] as $k3 => $v3) {
					...
					if(isset($detail['3'][$v3->glwnco])){
						foreach ($detail['3'][$v3->glwnco] as $k4 => $v4) {
								...
								foreach ($detail['4'][$v4->glwnco] as $k5 => $v5) {
									...
									if(isset($detail['5'][$v5->glwnco])){
										foreach ($detail['5'][$v5->glwnco] as $k6 => $v6) {
											...
											$edt_bln = check_edit($akses_ubah,$edit);
											$item6 .= '<td class="'.ck_edited($v6->cabang_edit,'biaya_bulan').'"><div style="min-height: 10px; width: 100%; overflow: hidden;" '.$edt_bln.' class="edit-value text-right edit-bulan" data-name="biaya_bulan" data-id="'.$v6->glwnco.'" data-value="'.$valueTxt.'">'.$valueTxt.'</div></td>';
									...
									$valueTxt = round_value($valueTxt);
									$edt_bln = check_edit($akses_ubah,$edit);
									$item5 .= '<td class="'.ck_edited($v5->cabang_edit,'biaya_bulan').'"><div style="min-height: 10px; width: 100%; overflow: hidden;" '.$edt_bln.' class="edit-value text-right edit-bulan" data-name="biaya_bulan" data-id="'.$v6->glwnco.'" data-value="'.$valueTxt.'">'.check_data($valueTxt,0).'</div></td>';
							...
							$edt_bln = check_edit($akses_ubah,$edit);
							$item4 .= '<td class="'.ck_edited($v4->cabang_edit,'biaya_bulan').'"><div style="min-height: 10px; width: 100%; overflow: hidden;" '.$edt_bln.' class="edit-value text-right edit-bulan" data-name="biaya_bulan" data-id="'.$v4->glwnco.'" data-value="'.check_data($valueTxt,$minus4).'">'.$valueTxt.'</div></td>';
					...
					$edt_bln = check_edit($akses_ubah,$editBulan3);
					$item3 .= '<td class="'.ck_edited($v3->cabang_edit,'biaya_bulan').'"><div style="min-height: 10px; width: 100%; overflow: hidden;" '.$edt_bln.' class="edit-value edit-bulan text-right" data-name="biaya_bulan" data-id="'.$v3->glwnco.'" data-value="'.$valueTxt.'">'.check_data($valueTxt,$minus3).'</div></td>';
			...
			if($v2->glwnco == '5721012'):
				$item2 .= '<td class="text-right">0</td>';
			else:
				$edt_bln = check_edit($akses_ubah,$editBulan2);
				$item2 .= '<td class="'.ck_edited($v2->cabang_edit,'biaya_bulan').'"><div style="min-height: 10px; width: 100%; overflow: hidden;"bulan2 '.$edt_bln.' class="edit-value edit-bulan text-right" data-name="biaya_bulan" data-id="'.$v2->glwnco.'" data-value="'.$valueTxt.'">'.check_data($valueTxt,$minus2).'</div></td>';
			endif;
	...
	$edt_bln = check_edit($akses_ubah,$editBulan1);
	$item .= '<td class="'.ck_edited($v->cabang_edit,'biaya_bulan').'"><div style="min-height: 10px; width: 100%; overflow: hidden;" '.$edt_bln.' class="edit-value text-right edit-bulan" data-name="biaya_bulan" data-id="'.$v->glwnco.'" data-value="'.$biaya_bulan.'">'.check_data($biaya_bulan,$minus).'</div></td>';
