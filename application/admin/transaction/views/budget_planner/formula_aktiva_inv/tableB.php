<?php
$dtE2 = json_encode($E4); $dtE2 = json_decode($dtE2,true);
$E2 = $E4;
$dataSaved = [];
$bac = 0;
$tambahBaru = 0;
...
$item = "<center>.</center><center>.";
    $test = [];
       foreach ($B as $val) {
        ...
        $keySaved = checkFormulaAkt2(['glwnco' => '5621014', 'tahun_core' => $anggaran->tahun_anggaran],$A_saved);
        if($keySaved['status']):
            $changed = json_decode($keySaved['data']['changed'],true);
            if(in_array('real_1', $changed)):
                $valB[0]['hasil'] = $keySaved['data']['real_1'];
            endif;
            if(in_array('real_2', $changed)):
                $valB[0]['hasil2'] = $keySaved['data']['real_2'];
            endif;
        endif;
        ...
        if(substr($val->glwnco, 0,4) == "1621"){
            ...
            foreach ($detail_tahun as $v => $val2) {
                if($val2->singkatan != arrSumberData()['real']):
                    $a = $v + 1;
                    $bulanLama = ($valB[0]['hasil']) - ($valB[0]['hasil2']); 
                    $keySaved = checkFormulaAkt(['glwnco' => '5621014_baru','tahun_core' => $val2->tahun],$A_saved,'bulan_'.$val2->bulan);
                    if($keySaved['status']):
                        $tPenyusutanBaru = $keySaved['data']['bulan_'.$val2->bulan];
                    else:
                        $keySaved = checkFormulaAkt(['glwnco' => '1621014'.'_baru','tahun_core' => $val2->tahun],$A_saved,'bulan_'.$val2->bulan);
                        if($keySaved['status']):
                            $tPenyusutanBaru += ($keySaved['data']['bulan_'.$val2->bulan]*0.5)/12;
                        else:
                            $key = checkFormulaAkt2(['bulan' => $val2->bulan,'tahun' => $val2->tahun],$dtE2);
                            if($key['status']):
                                $tPenyusutanBaru += ($key['data']['total']*0.5)/12;
                            endif;
                        endif;
                    endif;
        ...
        $item .= '</tr>';
        if($val->glwnco == '1621014'){
            $coa = $val->glwnco;
            $item .= '<tr>';

            $remove = '<button type="button" class="btn btn-danger btn-remove" data-id="1621014_baru" title="Hapus"><i class="fa-times"></i></button>';
            ...
            foreach ($detail_tahun as $v => $val) {
                if($val->singkatan != arrSumberData()['real']):
                    ...
                    $item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" '.$edit.' class="edit-value text-right " data-name="bulan_'.$val->bulan.'" data-id="'.($val->sumber_data).'-'.$coa.'_baru'.'-'.$val->tahun.'" data-value="'.view_report($hasilTB).'">'.custom_format(view_report($hasilTB)).'</div></td>';
                endif;
                
            }

            $item .= '</tr>';
        }else if($val->glwnco == '5621014'){
            $item .= '<tr>';
            $item .= '<td></td>';
            ...
            $remove = '<button type="button" class="btn btn-danger btn-remove" data-id="5621014_baru" title="Hapus"><i class="fa-times"></i></button>';
            if(!$access_edit) $remove = '';
            $item .= '<td class="button">'.$remove.'</td>';
