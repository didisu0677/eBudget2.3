...
$item = "<center>.";
    $test = [];
       foreach ($data as $val) {
        ...
        $keySaved = checkFormulaAkt2(['glwnco' => '5621016', 'tahun_core' => $anggaran->tahun_anggaran],$A_saved);
        if($keySaved['status']):
        ...
        if(substr($val->glwnco, 0,4) == "1621"){
            ...
            foreach ($detail_tahun as $v => $val2) {
                ...
                $keySaved = checkFormulaAkt(['glwnco' => '5621016_baru','tahun_core' => $val2->tahun],$A_saved,'bulan_'.$val2->bulan);
                if($keySaved['status']):
            ...
            foreach ($detail_tahun as $v => $val2) {
                ...
                $pengurang = 0;
                $keySaved = checkFormulaAkt(['glwnco' => '1621018'.'_baru','tahun_core' => $val2->tahun],$A_saved,'bulan_'.$val2->bulan);
                if($keySaved['status']):
        ...
        if($val->glwnco == '1621018'){
            ...
            $item .= '</tr>';
             $item .= '<tr>';
            $item .= '<td class="button"><button type="button" class="btn btn-danger btn-remove" data-id="5621016_baru" title="Hapus"><i class="fa-times"></i></button></td>';
