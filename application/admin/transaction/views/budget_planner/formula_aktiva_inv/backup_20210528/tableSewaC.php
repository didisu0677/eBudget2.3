...
$item = "<center>.";
    $test = [];
       foreach ($data as $val) {
        ...
        $keySaved = checkFormulaAkt2(['glwnco' => '5621017', 'tahun_core' => $anggaran->tahun_anggaran],$A_saved);
        if($keySaved['status']):
        ...
        if($val->glwnco == '1621019'){
            ...
            $item .= '</tr>';
             $item .= '<tr>';
            $item .= '<td class="button"><button type="button" class="btn btn-danger btn-remove" data-id="5621017_baru" title="Hapus"><i class="fa-times"></i></button></td>';
