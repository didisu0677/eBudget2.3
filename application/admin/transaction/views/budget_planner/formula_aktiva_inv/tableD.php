<?php
$dtE2 = json_encode($E3); $dtE2 = json_decode($dtE2,true);
$E2 = $E3;
$dataSaved = [];
$bac = 0;
$tambahBaru = 0;
...
$item = "<center>.</center><center>.";
    $test = [];
       foreach ($D as $val) {
        ...
        $keySaved = checkFormulaAkt2(['glwnco' => '5621012', 'tahun_core' => $anggaran->tahun_anggaran],$A_saved);
        if($keySaved['status']):
            $changed = json_decode($keySaved['data']['changed'],true);
            if(in_array('real_1', $changed)):
                $valD[0]['hasil'] = $keySaved['data']['real_1'];
            endif;
            if(in_array('real_2', $changed)):
                $valD[0]['hasil2'] = $keySaved['data']['real_2'];
            endif;
        endif;
        ...
        if(substr($val->glwnco, 0,4) == "1621"){
            ...
            foreach ($detail_tahun as $v => $val2) {
                if($val2->singkatan != arrSumberData()['real']):
                    $a = $v + 1;
                    $bulanLama = ($valD[0]['hasil']) - ($valD[0]['hasil2']); 
                    $keySaved = checkFormulaAkt(['glwnco' => '5621012_baru','tahun_core' => $val2->tahun],$A_saved,'bulan_'.$val2->bulan);
                    if($keySaved['status']):
                        $tPenyusutanBaru = $keySaved['data']['bulan_'.$val2->bulan];
                    else:
                        $keySaved = checkFormulaAkt(['glwnco' => '1621016'.'_baru','tahun_core' => $val2->tahun],$A_saved,'bulan_'.$val2->bulan);
                        if($keySaved['status']):
                            $tPenyusutanBaru += ($keySaved['data']['bulan_'.$val2->bulan]/36);
                        else:
                            $key = checkFormulaAkt2(['bulan' => $val2->bulan,'tahun' => $val2->tahun],$dtE2);
                            if($key['status']):
                                $tPenyusutanBaru += ($key['data']['total']/36);
                            endif;
                        endif;
                    endif;
        ...
        $item .= '</tr>';
        if($val->glwnco == '1621016'){
            $coa = $val->glwnco;
            $item .= '<tr>';

            $remove = '<button type="button" class="btn btn-danger btn-remove" data-id="1621016_baru" title="Hapus"><i class="fa-times"></i></button>';
            ...
            $remove = '<button type="button" class="btn btn-danger btn-remove" data-id="5621012_baru" title="Hapus"><i class="fa-times"></i></button>';
            if(!$access_edit) $remove = '';
            $item .= '<td class="button">'.$remove.'</td>';
