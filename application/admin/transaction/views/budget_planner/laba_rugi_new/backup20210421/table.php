...
foreach ($coa as $k => $v) {
				...
				foreach ($detail['2'][$v2->glwnco] as $k3 => $v3) {
						...
						foreach ($detail['3'][$v3->glwnco] as $k4 => $v4) {
								...
								foreach ($detail['4'][$v4->glwnco] as $k5 => $v5) {
									...
									for ($i=1; $i <= 12 ; $i++) { 
										if(count($dt6)>0){ 
											...
											$item5.= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;"  class="edited text-right" data-name="bulan_'.$i.'" data-id="'.$v5->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus5).'</div></td>';
							...
							for ($i=1; $i <= 12 ; $i++) {
								if(count($dt5)>0){ 
									...
									$item4.= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edited text-right" data-name="bulan_'.$i.'" data-id="'.$v4->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus4).'</div></td>';
								}
					...
					for ($i=1; $i <= 12 ; $i++) {
						if(count($dt4)>0){ 
							...
							$item3.= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;"  class="edited text-right" data-name="bulan_'.$i.'" data-id="'.$v3->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus3).'</div></td>';
	...
	for ($i=1; $i <= 12 ; $i++) { 
		if(count($dt2)>0){ 
			...
			$item .= '<td><div '.$idTr.' style="min-height: 10px; width: 100%; overflow: hidden;"  class="edited text-right" data-name="bulan_'.$i.'" data-id="'.$v->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus).'</div></td>';
			...
			if($v->glwnco == '4100000' || $v->glwnco == '4500000' || $v->glwnco == '4800000'){
				...
				if(isset($totalBiaya[$i])){ $totalBiaya[$i] += $val; }else{ $totalBiaya[$i] = $val; }
			}
			...
			$item .= '<td><div  '.$idTr.' style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value edited text-right" data-name="bulan_'.$i.'" data-id="'.$v->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus).'</div></td>';
