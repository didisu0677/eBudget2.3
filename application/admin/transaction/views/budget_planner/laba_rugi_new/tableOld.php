...
foreach ($coa as $k => $v) {
									...
									if(isset($detail['5'][$v5->glwnco])){
										foreach ($detail['5'][$v5->glwnco] as $k6 => $v6) {
											...
											for ($i=1; $i <=12 ; $i++) {
												...
												if(substr($v6->glwnco ,0,1) == '5' and $val > 0 ){
													$val = $val * -1;
												}
	...
	for ($i=1; $i <= 12 ; $i++) { 
		if(count($dt2)>0){ 
			...
			$item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;"  class="edited text-right" data-name="bulan_'.$i.'" data-id="'.$v->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus).'</div></td>';
			...
			$item .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="true" class="edit-value edited text-right" data-name="bulan_'.$i.'" data-id="'.$v->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus).'</div></td>';
		}
