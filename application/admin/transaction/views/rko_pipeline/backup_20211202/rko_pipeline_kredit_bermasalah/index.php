						...
						table_open('table table-striped table-bordered table-app',false,'','','data-table="tbl_m_produk"');
							thead('sticky-top');
								tr();
									...
									th(lang('sisa_outstanding').' ('.get_view_report().')','','rowspan="3" class="text-center align-middle min-150"');
									th(lang('kolektabilitas'),'','rowspan="3" class="text-center align-middle"');
									th(lang('produk_kredit'),'','rowspan="3" class="text-center align-middle min-150"');
									...
									th(str_replace('$$nama',$title,lang('perkiraan_giro')),'','rowspan="3" class="text-center align-middle min-150"');
								...
								tr();
								foreach ($arrWeekOfMonth['month'] as $k => $v) {
									th('','text-center align-middle d-month-'.$k,'colspan="'.$v.'" style="min-width:200px"');
								}
								tr();
		    ...
		    <table class="table table-bordered" id="form_table">
				<thead class="sticky-top">
					<tr>
						...
						<th class="text-center min-200 bg-grey">'.lang('sisa_outstanding').' ('.get_view_report().')'.'</th>
						<th class="text-center min-200 bg-grey">'.lang('kolektabilitas').'</th>
						<th class="text-center min-200 bg-grey">'.lang('produk_kredit').'</th>
...
function formOpen() {
	...
	if(typeof response_data.detail != 'undefined') {
		...
		$.each(list, function(k,v){
			...
			f.find('.sisa_outstanding').val(v.sisa_outstanding).trigger('change');
			f.find('.produk_kredit').val(v.produk_kredit).trigger('change');
			...
			f.find('.biaya').val(v.biaya).trigger('change');
		});
	}else{
		$('.btn-add-item').show();
	}
...
function add_item(){
	...
	item += `<td>
		...
		<input type="text" class="form-control keterangan" name="keterangan[]" data-validation="required" autocomplete="off"/>
		</td>`;
	item += '<td class="style-select2"><input type="text" class="text-right form-control sisa_outstanding money" name="sisa_outstanding[]" data-validation="required" /></td>';
	...
	item += '<td class="style-select2"><select class="form-control pilihan produk_kredit" name="produk_kredit[]" data-validation="required">'+dt_produk_kredit+'</select></td>';
	...
	item += '<td class="style-select2"><select class="form-control pilihan biaya" name="biaya[]" data-validation="required">'+dt_target+'</select></td>';
