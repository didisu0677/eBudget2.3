		...
		if(typeof $('#modal-form').attr('data-manual') == 'undefined') {
			...
			if($(this).data('id') == 0) {
				if($('#password').length > 0) {
					$('#password').attr('data-validation','required|min-length:3').removeAttr('placeholder');
				}
				if(autoTitle) {
...
// cookie set default cabang
$(document).on('change','#filter_cabang',function(){
	var val = $(this).val();
	setCookie("cabang",val,7);
});

$(document).on('change','#filter_cabang_induk',function(){
	var val = $(this).val();
	setCookie("cabang_induk",val,7);
	get_option_cabang(val);
});
$(document).on('change','.cookie_coa',function(){
	var val = $(this).val();
	setCookie("cookie_coa",val,7);
});

var xhr_ajax_cabang = null;
function get_option_cabang(val){
	if( xhr_ajax_cabang != null ) {
        xhr_ajax_cabang.abort();
        xhr_ajax_cabang = null;
    }

	xhr_ajax_cabang = $.ajax({
		url 	: base_url+'api/cabang_option',
		data 	: {
			parent 	: val,
			type 	: $('#filter_cabang_induk').attr('data-type'),
		},
		type	: 'post',
		dataType: 'json',
		success	: function(response) {
			xhr_ajax_cabang = null;
			$('#filter_cabang').html(response.data);
			var val = getCookie('cabang');
			if(val && $("#filter_cabang option[value='"+val+"']").length > 0){
				$('#filter_cabang').val(val).trigger('change');
			}else{
				val = $('#filter_cabang option:selected').val();
				$('#filter_cabang').val(val).trigger('change');
			}
		}
	});
...
$(document).ready(function(){
	...
	if(page_data && page_data.additional){
		var val = getCookie('cabang_induk');
		if(val && $("#filter_cabang_induk option[value='"+val+"']").length > 0){
			$('#filter_cabang_induk').val(val).trigger('change');
		}else{
			val = $('#filter_cabang_induk option:selected').val();
			get_option_cabang(val);
		}
	}else if (page_data && page_data.type == 'divisi'){
		$('#filter_cabang_induk').trigger('change');
	}else{
		var val = getCookie('cabang');
		if(val && $("#filter_cabang option[value='"+val+"']").length > 0){
			$('#filter_cabang').val(val).trigger('change');
	...
	if($('.cookie_coa').length>0){
		var val = getCookie('cookie_coa');
		if(val && $(".cookie_coa option[value='"+val+"']").length > 0){
			$('.cookie_coa').val(val).trigger('change');
		}
	}
...
function checkSubData(){
	for (var i = 1; i <= 6; i++) {
		if($(document).find('.sb-'+i).length>0){
...
function resize_window(){
	...
	var dt_height_window = $('body').find('.height-window').eq(0).data();
	var tab_app = $('body').find('.tab-app').eq(0).height();
	if(tab_app){
		height -= tab_app;
	}
	if(dt_height_window && dt_height_window.height){
		height -= dt_height_window.height;
	}
	height -= (170);
	// $('body').find('.height-window').height(height);
	$('body').find('.height-window').css({"max-height":height});
}

$( window ).resize(function() {
	var length = $('body').find('.height-window').length;
	if(length>0){
		resize_window();	
	}
});